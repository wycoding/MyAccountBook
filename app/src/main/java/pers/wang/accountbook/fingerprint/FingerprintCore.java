package pers.wang.accountbook.fingerprint;
...
public class FingerprintCore {
    ...
    public FingerprintCore(Context context) {
        ...
        Log.i("fingerprint isSupport: " , isSupport+"");
        initCryptoObject();
    }
    ...
    private void initCryptoObject() {
        try {
            ...
            Log.i("wy","create cryptoObject failed!");
        }
    }
    ...
    private void notifyAuthenticationSucceeded() {
        Log.i( "wy" , "onAuthenticationSucceeded");
    ...
    private void notifyAuthenticationError(int errMsgId, CharSequence errString) {
        Log.i( "wy" , "onAuthenticationError, errId:" + errMsgId + ", err:" + errString + ", retry after 30 seconds");
        if (null != mFpResultListener && null != mFpResultListener.get()) {
    ...
    private void notifyAuthenticationFailed(int msgId, String errString) {
        Log.i( "wy" , "onAuthenticationFailed, msdId: " +  msgId + " errString: " + errString);
        if (null != mFpResultListener && null != mFpResultListener.get()) {
    ...
    private void onFailedRetry(int msgId) {
        mFailedTimes++;
        Log.i( "wy" , "on failed retry time " + mFailedTimes);
        cancelAuthenticate();
    ...
    public static FingerprintManager getFingerprintManager(Context context) {
        ...
        try {
            ...
            Log.i( "wy" , "have not class FingerprintManager");
        }
